...
// ── Shake helper ─────────────────────────
function shakeTimeline(tl, target, axis = 'x') {
  return tl
    .set(target,  { [axis]: 0 })
    .to(target,   { [axis]: -16, duration: 0.06, ease: 'power2.out' })
    .to(target,   { [axis]:  16, duration: 0.08, ease: 'power2.inOut' })
    .to(target,   { [axis]:  -8, duration: 0.08, ease: 'power2.inOut' })
    .to(target,   { [axis]:   8, duration: 0.08, ease: 'power2.inOut' })
    .to(target,   { [axis]:   0, duration: 0.10, ease: 'power2.out' });
}
...
export default function FocusView() {
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      const next     = Math.min(maxScale, Math.max(MIN_SCALE,
                         scaleRef.current - e.deltaY * WHEEL_SCALE_SPEED));

      if (next === scaleRef.current) {
        const atMax = e.deltaY < 0 && next >= maxScale - 0.001;
        const atMin = e.deltaY > 0 && next <= 1.001;
        const noActive = !snapAnimRef.current?.isActive()
                      && !scaleAnimRef.current?.isActive();

        if ((atMax || atMin) && noActive) {
          snapAnimRef.current = shakeTimeline(gsap.timeline(), inner, 'x');
        }
        return;
      }
  ...
  return (
    <div
      ...
      style={isMobileRef.current ? { pointerEvents: 'none' } : undefined}
    >
      <div
        ref={innerRef}
